...
QuestOverlayUI = {
    ...
    -- Cache: questID -> true si tiene audio, false si no. Evita llamar PrepareSound
    -- en cada refresh del QuestLog (B10 performance fix).
    soundCache = {},
}
...
function QuestOverlayUI:CreatePlayButton(questID, ...)
    ...
    playButton:SetNormalTexture([[Interface\AddOns\QuestVoice\Textures\QuestLogPlayButton]])
    playButton:SetDisabledTexture([[Interface\AddOns\QuestVoice\Textures\QuestLogPlayButton]])
...
function QuestOverlayUI:UpdatePlayButtonTexture(questID, ...)
    ...
    if button then
        ...
        local texturePath = isPlaying and [[Interface\AddOns\QuestVoice\Textures\QuestLogStopButton]] or [[Interface\AddOns\QuestVoice\Textures\QuestLogPlayButton]]
...
function QuestOverlayUI:Update(...)
    ...
    for i = 1, QUESTS_DISPLAYED do
        ...
        if not isHeader then
            ...
            -- Cache check: evita llamar PrepareSound en cada refresh (B10)
            if self.soundCache[questID] == nil then
                local probe = { event = Enums.SoundEvent.QuestAccept, questID = questID }
                self.soundCache[questID] = DataModules:PrepareSound(probe) and true or false
            end

            if self.soundCache[questID] then
