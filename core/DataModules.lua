...
---@field LoadOnDemand boolean Whether the module can be dynamically loaded (TOC table.getn(table.getn(LoadOnDemand))
---@field ModuleVersion number Module's data format version (TOC table.getn(table.getn(X)-VoiceOver-DataModule-Version, must be CURRENT_MODULE_VERSION to be loaded)
---@field ModulePriority number Module's priority (TOC table.getn(table.getn(X)-VoiceOver-DataModule-Priority, larger number = higher priority)
---@field ContentVersion? string Module's content version (TOC table.getn(table.getn(Version))
---@field Title string Module's title (TOC table.getn(table.getn(Title) or addon name if missing)
---@field Maps number[] Map IDs in which the module should load (TOC table.getn(table.getn(X)-VoiceOver-DataModule-Maps)
...
---@field AddonName string Addon name
---@field Title string Module's title (TOC table.getn(table.getn(Title))
---@field ContentVersion string Module's content version (TOC table.getn(table.getn(Version))
...
DataModules =
{
    ...
    availableModules = {
        {
            AddonName = "QuestVoiceData",
            Title = "QuestVoice Data - Vanilla",
            ContentVersion = "0.1",
...
function DataModules:Register(name, module)
    ...
    if not metadata and (name == "QuestVoice" or name == "QuestVoiceData") then
        -- Turtle WoW (1.12) Bypass: Custom TOC metadata is not accessible via API.
        -- We manually reconstruct the metadata if this is the main module.
        metadata = {
            AddonName = name,
            LoadOnDemand = false,
    ...
    assert(metadata, format([[Module "%s" attempted to register but wasn't detected during addon enumeration]], name))

    local moduleVersion = tonumber(GetAddOnMetadata(name, "X-VoiceOver-DataModule-Version")) or (metadata and metadata.ModuleVersion)
    if not moduleVersion and name ~= "QuestVoice" then
         assert(false, format([[Module "%s" is missing data format version]], name))
    end

    -- Ideally if module format would ever change - there should be fallbacks in place to handle outdated formats
    assert((moduleVersion or CURRENT_MODULE_VERSION) == CURRENT_MODULE_VERSION,
        format([[Module "%s" contains outdated data format (version%d, expected%d)]], name, moduleVersion or 0,
...
-- We deliberately use a high table.getn(table.getn(Interface) version in TOC to ensure that all clients will load it.
