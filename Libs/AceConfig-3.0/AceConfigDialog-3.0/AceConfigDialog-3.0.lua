...
local gui = _G.QuestVoice_AceGUI
...
-- Polyfill para table.wipe (WoW 1.12)
...
-- Polyfill para strsplit (WoW 1.12)
		...
		for plugin, t in pairs(group.plugins) do
			for k, v in pairs(t) do
				if not opts[k] then
					...
					path[table.getn(path)+1] = k
					tempOrders[k] = GetOptionsMemberValue("order", v, options, path, appName)
					tempNames[k] = GetOptionsMemberValue("name", v, options, path, appName)
					path[table.getn(path)] = nil
				end
			end
		end
	end
...
do
	...
	if not frame or oldminor < 81 then
		...
		frame:SetWidth(320); frame:SetHeight(72)
		frame:EnableMouse(true) -- Do not allow click-through on the frame
		frame:SetFrameStrata("TOOLTIP")
		frame:SetFrameLevel(100) -- Lots of room to draw under it
		frame:SetScript("OnKeyDown", function(self, key)
			if key == "ESCAPE" then
				-- API removed for Vanilla
		...
		local function newButton(newText, ...)
			...
			button:SetWidth(128); button:SetHeight(21)
			-- API removed
			-- API removed
...
local function confirmPopup(appName, rootframe, basepath, info, message, func, ...)
	...
	frame.accept:SetScript("OnClick", function(self, ...)
		safecall(func, unpack(t)) -- Manually set count as unpack() stops on nil
			...
			if v.type == "group" then
					...
					if v.style == "radio" then
						...
						if not sorting then
							...
							for value, text in pairs(values) do
								sorting[table.getn(sorting)+1]=value
							end
				...
				elseif v.type == "multiselect" then
					...
					if control then
						...
						for s = 1, table.getn(valuesort) do
...
local function TreeOnButtonEnter(widget, event, uniquevalue, button)
	...
	local group = options
	for i = 1, table.getn(feedpath) do
...
function AceConfigDialog:FeedGroup(appName,options,container,rootframe,path, isRoot)
	...
	if hasChildGroups and not inline then
		...
		elseif (parenttype ~= "tree") or isRoot then
			...
			for i = 1, table.getn(treedefinition) do
...
local function RefreshOnUpdate(...)
	-- Usar el global 'this' de 1.12
	local frame = this
	if not frame or not frame.closing then return end
...
function AceConfigDialog:Open(appName, container, ...)
	...
	if type(container) == "table" and container.type == "BlizOptionsGroup" and table.getn(path) > 0 then
		for i = 1, table.getn(path) do
	...
	if container then
		...
		if not f then
			print("|cffff0000QuestVoice Error:|r Falló la creación del widget 'Frame'. Abortando apertura de ventana.")
			return
		end
		f:ReleaseChildren()
...
function AceConfigDialog:AddToBlizOptions(appName, name, parent, ...)
	...
	if not BlizOptions[appName][key] then
		...
		if not group then
			print("|cffff0000QuestVoice Error:|r Falló la creación de 'BlizOptionsGroup'. Verificando registro...")
			if not gui.WidgetRegistry["BlizOptionsGroup"] then
				print("|cffff0000QuestVoice Error:|r 'BlizOptionsGroup' NO está registrado en la instancia de AceGUI.")
			else
				print("|cffff0000QuestVoice Error:|r 'BlizOptionsGroup' está registrado pero el Constructor falló.")
			end
			return
		end
