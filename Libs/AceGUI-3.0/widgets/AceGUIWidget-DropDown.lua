...
local AceGUI = _G.QuestVoice_AceGUI or QuestVoice_AceGUI
if not AceGUI then return end
-- WoW 1.12 / Lua 5.0 polyfills
...
do
	...
	local function AddItem(self, item)
		self.items[table.getn(self.items) + 1] = item

		local h = table.getn(self.items) * 16
	...
	local function SetList(self, list, order, itemType)
		...
		if type(order) ~= "table" then
			for v in pairs(list) do
				sortlist[table.getn(sortlist) + 1] = v
			end
