...
function tracker.ButtonEvent(self, ...)
  ...
  if tracker.mode == "QUEST_TRACKING" then
    ...
    local colorperc = string.format("|cff%02x%02x%02x", math.floor(r * 255), math.floor(g * 255), math.floor(b * 255))
    ...
    self.perc = percent
    self.text:SetText(
      string.format("%s%s |cffaaaaaa(%s%%|cffaaaaaa)|r", showlevel, title or "", colorperc or "", ceil(percent))
    )
