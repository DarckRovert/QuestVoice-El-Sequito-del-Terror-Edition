...
pfUI.api.SetButtonFont = pfUI.api.SetButtonFont
  or function(button, font, size, flags)
...
pfUI.api.SetButtonFontColor = pfUI.api.SetButtonFontColor
  or function(button, r, g, b, a)
...
pfUI.api.strsplit = pfUI.api.strsplit
  or function(delimiter, subject)
    if not subject then
      return nil
    end
...
pfUI.api.SanitizePattern = pfUI.api.SanitizePattern
  or function(pattern, dbg)
...
pfUI.api.CreateBackdrop = pfUI.api.CreateBackdrop
  or function(f, inset, legacy, transp)
...
pfUI.api.SkinButton = pfUI.api.SkinButton
  or function(button, cr, cg, cb)
...
pfUI.api.CreateScrollFrame = pfUI.api.CreateScrollFrame
  or function(name, parent)
    ...
    f.slider.thumb:SetTexture(0.3, 1, 0.8, 0.5)
...
pfUI.api.CreateScrollChild = pfUI.api.CreateScrollChild
  or function(name, parent)
...
pfUI.api.round = pfUI.api.round
  or function(input, places)
    if not places then
      places = 0
    end
...
pfUI.api.rgbhex = pfUI.api.rgbhex
  or function(r, g, b, a)
    local key
...
pfUI.api.GetColorGradient = pfUI.api.GetColorGradient
  or function(perc, ...)
    ...
    return gradientcolors[index].r, gradientcolors[index].g, gradientcolors[index].b, gradientcolors[index].h
  end
...
pfUI.api.GetBestAnchor = pfUI.api.GetBestAnchor
  or function(self, ...)
...
pfUI.api.ConvertFrameAnchor = pfUI.api.ConvertFrameAnchor
  or function(self, anchor)
