...
SlashCmdList["PFDB"] = function(input, editbox)
  ...
  if input == "" or input == nil then
    DEFAULT_CHAT_FRAME:AddMessage("|cff33ffccQuest|rVoice (v" .. pfQuestConfig.version .. "):")
    ...
    DEFAULT_CHAT_FRAME:AddMessage(
      "|cff33ffcc/db|cffffffff show |cffcccccc - " .. pfQuest_Loc["Show database interface"]
    )
    DEFAULT_CHAT_FRAME:AddMessage(
      "|cff33ffcc/db|cffffffff config |cffcccccc - " .. pfQuest_Loc["Show configuration interface"]
    )
    DEFAULT_CHAT_FRAME:AddMessage(
    ...
    DEFAULT_CHAT_FRAME:AddMessage(
      "|cff33ffcc/db|cffffffff scan |cffcccccc - " .. pfQuest_Loc["Scan the server for custom items"]
    )
    DEFAULT_CHAT_FRAME:AddMessage(
  ...
  if arg1 == "debug" then
    ...
    DEFAULT_CHAT_FRAME:AddMessage(
      "|cff33ffccQuest|rVoice Debug Mode: " .. (pfQuest_config.debug and "|cff33ff33ON" or "|cffff3333OFF")
    )
  ...
  if arg1 == "track" or arg1 == "meta" then
    ...
    if not list or list == "" then
      ...
      DEFAULT_CHAT_FRAME:AddMessage(
        string.format(
          pfQuest_Loc['Available tracking targets are: %s. Or type "|cff33ffcc/db track clean|r" to untrack all.'],
          available
        )
      )
      return
  ...
  if deprecated[arg1] then
    DEFAULT_CHAT_FRAME:AddMessage(
      string.format(
        pfQuest_Loc['|cffffcc00WARNING:|r The command "|cff33ffcc/db%s|r" is deprecated and will be removed soon. Please use the "|cff33ffcc/db track%s|r" instead to achieve the same functionality.'],
        arg1,
        arg1
      )
    )
  ...
  if arg1 == "lock" then
    ...
    DEFAULT_CHAT_FRAME:AddMessage(
      "|cff33ffccQuest|rVoice Tracker: " .. (pfQuest_config.lock and "Locked" or "Unlocked")
    )
    return
  end
